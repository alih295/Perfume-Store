...
function CartSummary({ item }) {
  ...
  return (
    <motion.div
      whileHover={{ scale: 1.01, boxShadow: "4px 4px 10px rgba(0,0,0,0.7)" }}
      transition={{ duration: 0.15 }}
      className="w-full relative   flex h-60 items-center   justify-evenly  p-5 "
    >
      <div
        onClick={() => {
          removeItems(item._id);
        }}
        className="text-lg absolute right-5 cursor-pointer top-10"
      >
        <GiCrossedBones />
      </div>
      <img
        className=" w-60 object-cover rounded-lg h-full "
